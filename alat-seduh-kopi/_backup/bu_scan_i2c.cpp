...
// Diperlukan untuk menggunakan `std::vector` untuk menyimpan alamat I2C.
#include <vector>
...
// Jumlah baris pada LCD (4 untuk 4).
#define LCD_ROWS    4
// Membuat objek LCD, dengan parameter alamat I2C, jumlah kolom, dan jumlah baris.
LiquidCrystal_I2C lcd(LCD_ADDRESS, LCD_COLUMNS, LCD_ROWS);

// --- GLOBAL VARIABLE UNTUK MENYIMPAN HASIL I2C SCAN ---
// Ini akan menyimpan string JSON dari hasil scan I2C.
...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client,
    ...
    if(type == WS_EVT_CONNECT){
        ...
        jsonRelay += motorState; // Tambahkan nilai boolean (true/false)
        jsonRelay += "}";
        client->text(jsonRelay); // Kirim sebagai teks JSON

        // --- KIRIM HASIL I2C SCAN KE KLIEN YANG BARU TERHUBUNG ---
        client->text(i2cScanResultsJson);
    }
    // Jika klien terputus
...
// --- Bagian 8: Fungsi I2C Scanner ---
// Fungsi ini memindai bus I2C dan mencetak alamat perangkat yang ditemukan ke Serial Monitor dan LCD.
// Sekarang juga mengembalikan daftar alamat yang ditemukan.
std::vector<String> i2cScanner() {
  Serial.println("\n--- Memulai I2C Scanner ---");
  lcd.clear(); // Bersihkan LCD
  lcd.setCursor(0, 0);
  lcd.print("Scanning I2C..."); // Tampilkan pesan di LCD

  byte error, address;
  int nDevices = 0; // Counter perangkat yang ditemukan
  std::vector<String> foundAddresses; // Vector untuk menyimpan alamat yang ditemukan

  // Loop melalui semua kemungkinan alamat I2C (dari 1 hingga 127)
  for(address = 1; address < 127; address++ ) {
    Wire.beginTransmission(address); // Mulai transmisi ke alamat ini
    error = Wire.endTransmission(); // Akhiri transmisi dan cek respons

    if (error == 0) { // Jika tidak ada error (ACK diterima)
      String addrStr = "0x";
      if (address < 16) addrStr += "0"; // Format agar selalu dua digit
      addrStr += String(address, HEX);

      foundAddresses.push_back(addrStr); // Simpan alamat yang ditemukan

      Serial.print("I2C device found at address ");
      Serial.println(addrStr);

      // Tampilkan alamat di LCD, atur posisi agar tidak tumpang tindih
      lcd.setCursor(nDevices % 2 == 0 ? 0 : 10, (nDevices / 2) + 1); // Baris 1 atau 2, kolom 0 atau 10
      lcd.print(addrStr);
      nDevices++;
      if (nDevices >= 4) { // Jika sudah 4 perangkat, jeda sebentar agar bisa terlihat
          delay(1000);
          lcd.clear();
          lcd.setCursor(0,0);
          lcd.print("Scanning I2C (cont)...");
          nDevices = 0; // Reset counter untuk penempatan di baris LCD
      }
    }
  }
  ...
  if (foundAddresses.empty()) {
    ...
    lcd.setCursor(0, 0); // Kembali ke baris 0
    lcd.print("I2C Found: ");
    lcd.print(foundAddresses.size());
    lcd.setCursor(0, 1); // Bersihkan baris di bawah
    lcd.print("                 ");
    delay(2000); // Tampilkan ringkasan sebentar
  }
  Serial.println("\n--- I2C Scanner Selesai ---");
  delay(1000); // Memberi waktu untuk membaca hasil scan

  return foundAddresses; // Kembalikan vector alamat
}


// --- Bagian 9: Fungsi Setup (Inisialisasi) ---
void setup() {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Booting...");
    delay(1000); // Beri waktu untuk membaca pesan

    // Panggil fungsi I2C Scanner di awal startup
    std::vector<String> addresses = i2cScanner(); // Jalankan scan dan simpan hasilnya
    ...
    i2cScanResultsJson += "]}";
    Serial.println("I2C Scan JSON untuk Web: " + i2cScanResultsJson); // Untuk debugging


    // --- Lanjutkan proses setup setelah scan I2C ---
    ...
    lcd.setCursor(0, 0); // Bersihkan dan tampilkan pesan baru di LCD
    ...
    while(WiFi.status() != WL_CONNECTED){
        ...
        Serial.print(".");
        // Tampilkan animasi titik di LCD
        lcd.print(".");
        wifi_dot_count++;
        if (wifi_dot_count > 10) { // Reset titik jika terlalu panjang
            lcd.setCursor(0, 1);
...
// --- Bagian 10: Fungsi Loop (Eksekusi Berulang) ---
void loop() {
