...
void loop() {
    ...
    // --- Panggil Fungsi Handle dari Komponen Order Coffee ---
    handleOrderCoffee(); // <<< UBAH PANGGILAN FUNGSI INI

    // --- Panggil Fungsi Handle dari Komponen RFID Card Reader ---
    handleRfidCardReader(currentMillis);

    // --- Panggil Fungsi Handle dari Komponen DHT22 ---
    ...
    if (!menuActive && !menuConfirmed) { // <<< Variabel ini sekarang extern dari order_coffee.h
        if(currentMillis - lastSensorReadMillis >= sensorReadInterval){
            ...
            // Baris 0: "Coffee Wd"
            // Baris 1: Digunakan untuk data sensor secara rotasi
            // Baris 2: "Silahkan pilih menu"
            // Baris 3: "kopi pilihan anda!"

            // Pesan statis (sudah Anda pindahkan)
            lcd.setCursor(0, 0);
            lcd.print("Coffee Wd");
            lcd.setCursor(0, 2);
            lcd.print("Silahkan pilih menu");
